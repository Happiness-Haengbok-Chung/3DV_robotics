...
EXPERT_MODEL_PATH = "DDPG_fetchreach_expert"
...
def make_env(render_mode): # 로봇 팔을 책상 위에 가져다 놓고 전원을 넣는 것
    ...
    """
    env = gym.make(ENV_ID,reward_type="sparse", render_mode=render_mode) # 로봇 시뮬레이터, MJCF 파일 (MuJoCo의 로봇/환경 정의 파일) 불러오고, 물리 엔진 초기화하고, reward function 세팅하고, action space / observation space 세팅하고
...
class SuccessEvalCallback(BaseCallback):
    """
    DDPG 학습 중 주기적으로 성공률 평가해서 기록하는 콜백.
    """
    def __init__(self, eval_env, n_eval_episodes=20, eval_freq=50_000, verbose=0):
        super().__init__(verbose)
        self.eval_env = eval_env
        self.n_eval_episodes = n_eval_episodes
        self.eval_freq = eval_freq
        self.eval_results = []  # (num_timesteps, success_rate) 리스트

    def _on_step(self) -> bool:
        # n_calls: callback이 호출된 횟수, num_timesteps: 전체 환경 step 수
        if self.n_calls % self.eval_freq == 0:
            ...
            if self.verbose > 0:
                print(f"[Expert Eval] steps={self.num_timesteps}, success={success_rate:.3f}")
        return True

# ===========================
# 1. Expert (DDPG) 학습 + 데이터 수집
# ===========================
def train_expert(total_timesteps: int = 2_000_000, tuning: bool = True, max_episode_length: int = 150):
    ...
    action_noise = NormalActionNoise(
        ...
        sigma=0.25 * np.ones(n_actions),   # 0.1~0.3 정도 범위에서 튜닝
    )
    ...
    if tuning and os.path.exists(EXPERT_MODEL_PATH + ".zip"):
        ...
        model = DDPG.load(EXPERT_MODEL_PATH, env=env)
        model.set_env(env)
        model.learn(
        ...
        model = DDPG(
            "MultiInputPolicy",
            env,
            gamma=0.95,                    # Fetch 계열에서 자주 쓰는 값
            learning_rate=1e-3,
            buffer_size=int(1e6),
            batch_size=256,
            tau=0.05,
            train_freq=(1, "episode"),     # 또는 (1, "step")
            gradient_steps=50,
            ...
            replay_buffer_kwargs=dict(
                ...
                # online_sampling=True,      # HER에서 online sampling
            ),
            verbose=1,
        )
        model.learn(
...
def flatten_obs(obs_dict):
    # FlattenObservation이 하는 것과 비슷하게 합쳐줌
...
def collect_expert_data(
    ...
    env = make_env(render_mode=None)
    model = DDPG.load(EXPERT_MODEL_PATH, env=env) # Example
...
def train_diffusion_policy(
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if epoch % 5 == 0:
            success_rate = eval_diffusion_success(model, diffusion, n_episodes=10, max_steps=50)
            diffusion_eval_log.append((epoch, success_rate))
            print(f"[Diffusion Eval] epoch={epoch}, success={success_rate:.3f}")

    print(f"✅ Diffusion policy saved to: {DIFFUSION_MODEL_PATH}")
    return model, diffusion, cfg, diffusion_eval_log
...
def evaluate_expert():
    print("🔎 Evaluating DDPG Expert policy...")
...
def eval_diffusion_success(model: DiffusionPolicy, diffusion: ActionDiffusion,
                           n_episodes: int = 10, max_steps: int = 50) -> float:
    """
    렌더 없이 diffusion policy 성공률만 빠르게 평가.
    """
    env = make_env(render_mode=None)
    success = 0.0
...
def record_expert_video(
    ...
    """
    DDPG Expert 정책이 수행하는 궤적을 GT처럼 GIF로 저장.
    """
    assert os.path.exists(EXPERT_MODEL_PATH + ".zip"), "Expert not trained yet!"
...
if __name__ == "__main__":
    ...
    diffusion_log = None
    if TRAIN_EXPERT:
        expert_model, expert_log = train_expert(total_timesteps=10e6, tuning=True)
        evaluate_expert()
        record_expert_video(n_episodes=5, max_steps=150, video_dir="videos_expert")

    if COLLECT_DATA:
        collect_expert_data(n_episodes=200, max_steps_per_episode=50)

    if TRAIN_DIFFUSION:
        model, diffusion, cfg, diffusion_log = train_diffusion_policy(
            batch_size=256,
            epochs=130,
            lr=1e-4,
            resume=True,
    ...
    if (expert_log is not None and len(expert_log) > 0) or \
       (diffusion_log is not None and len(diffusion_log) > 0):

        plt.figure(figsize=(6,4))

        # Expert (DDPG) 성공률
        if expert_log is not None and len(expert_log) > 0:
            steps, expert_succ = zip(*expert_log)
            plt.plot(steps, expert_succ, label="DDPG Expert", marker="o")

        # Diffusion policy 성공률 (epoch 단위)
        if diffusion_log is not None and len(diffusion_log) > 0:
            epochs, diff_succ = zip(*diffusion_log)
            plt.plot(epochs, diff_succ, label="Diffusion Policy", marker="x")

        plt.xlabel("Steps (expert) / Epochs (diffusion)")
