...
class MuJoCoRenderer:
    ...
    def __init__(self, env):
        ...
        if isinstance(obs_space, spaces.Dict):
            # sum all component dims
            self.observation_dim = (
                sum(int(np.prod(space.shape)) for space in obs_space.values()) - 1
            )
        else:
    ...
    def pad_observations(self, observations):
        """
        Compute (xpos, obs) when the MuJoCo env does NOT expose `sim`
        (e.g., gym-robotics FetchReach-v4).

        Falls back to env.sim.data if available; otherwise uses env.data.
        """

        # unwrap wrappers
        env = self.env.unwrapped if hasattr(self.env, "unwrapped") else self.env

        # --- get mujoco state safely ---
        if hasattr(env, "sim") and hasattr(env.sim, "data"):
            mj_data = env.sim.data                     # old mujoco_py style
        elif hasattr(env, "data") and hasattr(env, "model"):
            mj_data = env.data                         # new gym-robotics style
        else:
            raise AttributeError(
                "Env has neither sim.data nor data – cannot infer qpos/qvel for pad_observations."
            )

        # determine qpos dimension correctly
        qpos_dim = mj_data.qpos.size

        ## In FetchReach, the 'observations' passed in are already flattened
        ## and DO NOT include the xpos state → so infer xpos from velocity.
        xvel_dim = qpos_dim - 1                       # original logic preserved

        # we must check dimension safety
        assert observations.shape[1] > xvel_dim, \
            f"Observation dim={observations.shape[1]} is too small to index xvel at {xvel_dim}"

        # extract velocities + integrate to get xpos
        xvel = observations[:, xvel_dim]
        xpos = np.cumsum(xvel) * env.dt               # env.dt works for both v3/v4

        # prepend xpos back into the observation vector
        states = np.concatenate(
            [xpos[:, None], observations], axis=-1
        )
...
def rollout_from_state(env, state, actions):
    ...
    base_env.set_state(state[:qpos_dim], state[qpos_dim:])
    observations = [base_env._get_obs()]
    for act in actions:
        obs, rew, term, _ = base_env.step(act)
