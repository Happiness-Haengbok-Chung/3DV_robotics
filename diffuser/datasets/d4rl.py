...
def get_dataset(env):
    if env.spec.id == "FetchReach-v3":
        data = np.load("fetchreach_d4rl_dataset.npz")
        dataset = {
    ...
    if env.spec.id == "FetchPickAndPlace-v2":
        data = np.load("pp_d4rl_dataset.npz")
        dataset = {
            "observations": data["observations"],
            "actions": data["actions"],
            "rewards": data["rewards"],
...
def sequence_dataset(env, preprocess_fn):
    ...
    assert use_timeouts, dataset.keys()
